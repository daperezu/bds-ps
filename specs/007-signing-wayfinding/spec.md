...
**Amended**: 2026-04-24
**Trigger**: Post-ship field report. Once an applicant submits their response and the application transitions to `ResponseFinalized`, the application disappears from every reviewer-visible list on `/Review`. The **Initial Review Queue** filters to `Submitted` only (see `src/FundingPlatform.Application/Services/ReviewService.cs:24-27`); the **Signing Inbox** filters to applications that already have a pending signed upload (see `src/FundingPlatform.Infrastructure/Persistence/Repositories/SignedUploadRepository.cs:42`), which requires a Funding Agreement to already exist. The agreement-generation action (`FundingAgreementController.Generate`, authorized for any user with the `Reviewer` or `Admin` role — see `src/FundingPlatform.Web/Controllers/FundingAgreementController.cs:100-102` and `src/FundingPlatform.Domain/Entities/Application.cs:462-467`) therefore has no discoverable entry point from any reviewer-accessible page. `/Application/Details/{id}` is `[Authorize(Roles = "Applicant")]` (see `src/FundingPlatform.Web/Controllers/ApplicationController.cs:12-13`) and cannot serve as a reviewer entry either. Result: generation is a dead-letter action whose preconditions can be met but whose trigger is effectively unreachable without URL guessing, which in turn leaves every applicant stuck on the *"No agreement has been generated yet."* state covered by acceptance scenario 3 of User Story 2.

**Scope of the amendment**: Add a third `/Review` sub-tab — a peer to the two introduced in FR-001 — that lists applications awaiting Funding Agreement generation and links each row to the existing `/Applications/{applicationId}/FundingAgreement` page, where the Generate button is already exposed to users with the `Reviewer` or `Admin` role per spec 005. No new server-side generation entry point, no change to authorization of existing surfaces, no schema change.

**Authorization model — calibration against the existing codebase**: Verification performed during planning (see *Amendment Assumptions* below) confirmed that **per-application reviewer assignment does not exist in the codebase today**. The sibling queues — **Initial Review Queue** and **Signing Inbox** — are role-based: any user with the `Reviewer` role sees every row in either queue; admins likewise see everything. The domain method `Application.CanUserGenerateFundingAgreement(isAdministrator, isReviewerAssignedToThisApplication)` names its second parameter as if per-application assignment existed, but every call site populates it with `User.IsInRole("Reviewer")`, a pure role check (see `src/FundingPlatform.Web/Controllers/FundingAgreementController.cs:100-102`). This amendment adopts the same role-based model as the sibling queues. Introducing per-application assignment — schema change, admin assignment UI, backfill — is explicitly deferred to a future feature and is listed in *Amendment Out of Scope*.
...
A user with the `Reviewer` role signs in after an applicant has submitted their response. They click **Review** in the main navigation and see a new sub-tab labeled **Generate Agreement** positioned between **Initial Review Queue** and **Signing Inbox**. Selecting it shows a paginated list of every application in state `ResponseFinalized` with no Funding Agreement record yet, with columns *Application # · Applicant · Response finalized on · Open →*. The reviewer clicks **Open** on a row and lands on `/Applications/{applicationId}/FundingAgreement`, where the Generate button shipped by spec 005 is already visible to their role. After they generate the agreement, the same application leaves this queue on the next page load and becomes visible to the applicant via the embedded panel on `/ApplicantResponse/Index/{id}` (as already covered by User Story 2), closing the end-to-end wayfinding loop.
...
**Independent Test**: Seed an application at `ResponseFinalized` with no Funding Agreement. Starting from `/`, a user with the `Reviewer` role reaches the Generate Agreement tab in at most two clicks (**Review** → **Generate Agreement**) and sees the application listed. Clicking **Open** lands on the Funding Agreement page with the Generate button visible. An `Admin` user sees the same list.

**Acceptance Scenarios**:

1. **Given** a user with the `Reviewer` or `Admin` role, **When** they navigate to `/Review` and click the **Generate Agreement** sub-tab, **Then** the page lists every application in state `ResponseFinalized` with no Funding Agreement record, with columns *Application #*, *Applicant*, *Response finalized on*, and an **Open** action linking to `/Applications/{applicationId}/FundingAgreement`.
2. **Given** a user viewing the **Generate Agreement** tab, **When** they click **Open** on a row and then click **Generate agreement** on the Funding Agreement page, **Then** the application is removed from the **Generate Agreement** tab on the next page load, and — per User Story 2 — appears on the applicant's response page with the *"ready to sign"* banner.
3. **Given** a user with the `Reviewer` or `Admin` role and no applications matching the tab's criteria, **When** they open **Generate Agreement**, **Then** the empty-state message *"No applications are waiting for agreement generation."* renders with the test hook `data-testid="generate-agreement-empty"`.
4. **Given** an application in state `AppealOpen` with no Funding Agreement, **When** a reviewer opens **Generate Agreement**, **Then** the application does NOT appear in the list (`AppealOpen` is a distinct state; its discoverability is explicitly out of scope for this amendment).
5. **Given** an application in state `ResponseFinalized` whose applicant rejected every item (so `Application.CanGenerateFundingAgreement()` would return false on the preconditions check), **When** a reviewer opens **Generate Agreement**, **Then** the application DOES appear in the list — the tab's filter is scoped by state plus missing agreement only. The Funding Agreement page surfaces the precondition blocker when the reviewer clicks **Open**.
6. **Given** a user with only the `Applicant` role, **When** they attempt to reach `/Review/GenerateAgreement` directly, **Then** access is denied, matching the existing `/Review` authorization posture.
7. **Given** two applications — one finalized earlier and one finalized later — both matching the tab's criteria, **When** a reviewer opens **Generate Agreement**, **Then** the earlier-finalized application appears above the later-finalized one (ascending order of response-finalized-on so long-waiting applicants are not starved).
...
- **FR-009**: The **Generate Agreement** tab MUST list applications matching ALL of the following, paginated using the same `PageSize = 25` constant as `ReviewService.GetReviewQueueAsync` (see `src/FundingPlatform.Application/Services/ReviewService.cs:16`), in ascending order of response-finalized-on (oldest first):
  - Application state equals `ResponseFinalized`; AND
  - The application has no associated Funding Agreement record.

  No per-user filtering is applied. Any signed-in user who passes the route-level authorization (i.e., holds the `Reviewer` or `Admin` role) sees every application matching the two clauses above. This mirrors the authorization posture of the sibling **Initial Review Queue** and **Signing Inbox** tabs, which today filter by state / upload-status only and never by per-application reviewer assignment.
- **FR-010**: Each row in the **Generate Agreement** tab MUST display the application identifier (formatted as `#{ApplicationId}` to match the sibling tabs), the applicant's display name, the timestamp the response was finalized, and an **Open** action. The **Open** action MUST route to `/Applications/{applicationId}/FundingAgreement` — the same target used by the **Signing Inbox**'s Open link per the existing implementation (`src/FundingPlatform.Web/Views/Review/SigningInbox.cshtml:52-56`) — so reviewers reach a page where the Generate button is already role-gated and ready.
...
- **SC-008**: A signed-in user with the `Reviewer` or `Admin` role reaches an application ready for agreement generation in at most three clicks from any signed-in page: **Review** → **Generate Agreement** → **Open**. No URL typing required.
- **SC-009**: Any signed-in user with the `Reviewer` or `Admin` role sees every application matching FR-009 in their **Generate Agreement** tab. No user-specific filtering is applied, consistent with the sibling tabs.
- **SC-010**: The Playwright coverage for User Story 4 is added alongside the SC-007 suite and includes: (a) a reviewer sees a seeded `ResponseFinalized`-without-agreement application in the **Generate Agreement** tab with the correct columns and sort order; (b) an admin sees the same list; (c) the empty-state message and its `data-testid` render when no rows match; (d) the end-to-end chain from **Generate Agreement** → **Open** → **Generate** → the applicant's embedded panel per User Story 2 completes without navigating off the `/Review/*` and `/ApplicantResponse/Index/{id}` surfaces.
...
- The response-finalization timestamp source is `ApplicantResponse.SubmittedAt` on the latest `ApplicantResponse` belonging to the application (planning-time verification confirms no dedicated `ResponseFinalizedAt` field exists on `Application`; `ApplicantResponse.Submit` stamps `SubmittedAt = DateTime.UtcNow` at the exact transition into `ResponseFinalized` per `src/FundingPlatform.Domain/Entities/ApplicantResponse.cs:51` called from `Application.SubmitResponse` at `src/FundingPlatform.Domain/Entities/Application.cs:207-232`). The list sort and displayed column use this timestamp.
- **Per-application reviewer assignment does not exist in the codebase today.** The domain method `Application.CanUserGenerateFundingAgreement(isAdministrator, isReviewerAssignedToThisApplication)` names its second parameter as if assignment existed, but every call site populates it with `User.IsInRole("Reviewer")` — a pure role check (see `src/FundingPlatform.Web/Controllers/FundingAgreementController.cs:100-102`). The sibling queues (**Initial Review Queue** and **Signing Inbox**) are likewise role-based with no per-application scoping — notably `SignedUploadRepository.GetPendingInboxAsync` accepts `reviewerUserId` and `isAdmin` parameters but does not use them in its WHERE clause (`src/FundingPlatform.Infrastructure/Persistence/Repositories/SignedUploadRepository.cs:25-74`). This amendment adopts the same role-based model and treats per-application assignment as a future feature (see *Amendment Out of Scope*).
- The `/Applications/{applicationId}/FundingAgreement` page already exposes the **Generate** button to any `Reviewer` or `Admin` user when `Application.CanGenerateFundingAgreement()` preconditions are met. This amendment does not change that.
- Pagination on the sibling tabs uses the shared `PageSize = 25` constant on `ReviewService` plus inline (non-partial) pagination markup in each Razor view. The new tab adopts the same convention rather than introducing a shared pagination partial.

### Amendment Out of Scope

- **Per-application reviewer assignment.** Introducing a `ReviewerAssignment` (or equivalent) entity, the join table, admin UI to assign reviewers to applications, backfill of existing applications, and the corresponding authorization upgrade to `FundingAgreementController.Generate`. The domain's `isReviewerAssignedToThisApplication` parameter remains aspirational. If/when per-reviewer ownership becomes a product requirement, it is a standalone feature (candidate feature 008 or later) that would then also tighten the **Initial Review Queue** and **Signing Inbox** scoping.
